...
char pro;
  ...
  while (irakurri_agindua(prompt, buf_agindua) > 0)
  {
	...
	if (strcmp(agindua_arg_formatuan[0], "echo") == 0){
		
		if ( strchr(buf_agindua, '>') == NULL){
		printf_trace(gurasoa, "echo comando ez baliogabea\n");
		continue;
		}	
		char *c = agindua_arg_formatuan[1];
		int i = 1;
		char text[300] = "";
		while (strcmp(c, ">") != 0){
			if (strcmp(c, "\"") != 0){
			...
			i++;
			c = agindua_arg_formatuan[i];
			if (strcmp(c, ">") != 0){
			strcat(text, " ");
			}
			
		}
		printf_trace(gurasoa, "> aurkitua %d posizioan\n", i);
		char *where = agindua_arg_formatuan[i+1];
		char *symbol = agindua_arg_formatuan[i];
		printf_trace(gurasoa, "read text: %s\n", text, symbol, where);
		FILE *file = fopen(where, "w+");
		if (file == NULL){
		errorea_tratatu("Fitxategia ezin izan da zabaldu\n");
		}
		fprintf(file, "%s", text);
		fclose(file);
		continue;
