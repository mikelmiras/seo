...
ssize_t irakurri_agindua(char *prompt, char *agindua)
{
  ...
  if (karaktere_kop==0) 
  {
    printf_trace(false, "Aginduen bukaera => ateratzen");
    return 0;
  }
  ...
  // Debug: idatzi teklatutik irakurritako karaktere-katea (agindua)
    ...
    printf_trace(false, "Irakurritako agindua string formatuan: %s\n", mezua_trace);
  #endif
...
int eskuratu_argumentuak(char *agindua_string, char *arg_array[])
{
  ...
  argumentua = strtok(agindua_string, arg_bereizleak); 
...
void printf_trace (bool umea, const char * format, ... ) 
{
    ...
    if (umea==false) fprintf(stderr, COLOR_KOIN "[DEBUG] koin (pid %d): ", getpid());
